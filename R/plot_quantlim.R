...
plot_quantlim <- function(spikeindata, quantlim_out, alpha, dir_output, xlim_plot){
  ...
  unique_c = sort(unique(expdata$C));   # list unique concentration points
  iii = 1
  for (j in unique_c){
    data_f <- subset(expdata, C == j)
  ...
  p1 <- p1 + xlab('Protein copies on column') + ylab('Integrated peak area')  # LKP - CHANGED AXIS LABELS
